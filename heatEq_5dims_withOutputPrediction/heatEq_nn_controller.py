...
# Learn the gradient descent function to directly predict u
class uOpt(nn.Module):
    def __init__(self, u_dim, hidden_size=8):
        super(uOpt, self).__init__()

        # Input is the cost and constraint nodes
        self.input = nn.Linear(2, hidden_size)
        self.h1 = nn.Linear(hidden_size, hidden_size)
        self.h2 = nn.Linear(hidden_size, hidden_size)
        self.h3 = nn.Linear(hidden_size, hidden_size)
        self.h4 = nn.Linear(hidden_size, u_dim)
    ...
    def forward(self, ctg_in, cst_in):
        ctg_cst = torch.hstack((ctg_in, cst_in))

        u_opt1 = F.leaky_relu(self.input(ctg_cst))
        u_opt2 = F.leaky_relu(self.h1(u_opt1))
        u_opt3 = F.leaky_relu(self.h2(u_opt2))
        u_opt4 = F.leaky_relu(self.h3(u_opt3))
        u_opt_out = F.leaky_relu(self.h4(u_opt4))
...
class HeatEqNNController:
    def __init__(self, x_dim, x_rom_dim, u_dim):
        ...
        self.u_opt = uOpt(u_dim, hidden_size=8)
    ...
    def fit(self, dataframe):
        ...
        # Generate the u_opt columns by doing basinhopping on the trained 1st half
        # Internal function
        def _basinhopper_helper(u_bh, *arg_x_bh):
            x_bh = arg_x_bh[0]
            ...
            # This should however be alleviated by the basinhopping
            if cst_pred_bh > 0:
        ...
        def _basinhopper(_x):
            ...
            result = optimize.basinhopping(
                func=_basinhopper_helper, x0=[273, 273], niter=2, minimizer_kwargs=min_kwargs
            )
            ...
            if u_opt[1] > 373:
                ...
                u_opt[1] = 73
            print("u_opt found: {}".format(u_opt))
            return u_opt

        u_opt_data = []
        # x is a tensor with num_traj * 10 rows and 20 columns
        for x_row in x:
            u_opt_data.append(_basinhopper(x_row))
        # Convert u_opt into a tensor for training
        u_opt_data = np.array(u_opt_data, dtype=np.float32)
        u_opt_data = self.scaler_u.transform(u_opt_data)
        u_opt_tensor = torch.tensor(u_opt_data)

        # Train the 2nd half of the neural net to predict the optimal u
        self.u_opt.train()

        minibatch2 = torch.utils.data.TensorDataset(u_opt_tensor, x)
        mb_loader2 = torch.utils.data.DataLoader(minibatch2, batch_size=120, shuffle=False)

        # 2nd training loop for the 2nd half
        u_opt_optimizer = optim.SGD(self.u_opt.parameters(), lr=0.05)
        u_opt_criterion = nn.MSELoss()

        for epoch in range(10):
            for u_opt_mb, x_mb in mb_loader2:
                u_opt_optimizer.zero_grad()

                self.x_mor.eval()
                self.net.eval()
                with torch.no_grad():
                    x_rom_mb = self.x_mor(x_mb)
                    ctg_mb, cst_mb = self.net(x_rom_mb, u_opt_mb)

                u_opt_pred = self.u_opt(ctg_mb, cst_mb)
                loss_u = u_opt_criterion(u_opt_pred, u_opt_mb)
                loss_u.backward()
                u_opt_optimizer.step()

            self.u_opt.eval()
            with torch.no_grad():
                u_opt_pred = self.u_opt(ctg, cst)
                loss_u = u_opt_criterion(u_opt_pred, u_opt_tensor)
                print("Epoch {}: loss_u = {}".format(epoch, loss_u))
            self.u_opt.train()
...
if __name__ == "__main__":
    data = pd.read_csv(data_folder + "heatEq_10_trajectories_train_basinhopper.csv")
    heatEq_nn = HeatEqNNController(x_dim=20, x_rom_dim=5, u_dim=2)
    heatEq_nn.fit(data)

    with open("heatEq_nn_controller_5dim_uOpt.pickle", "wb") as pickle_file:
